...
sessionInfo = {"tokenUsage": {"outbound": 0, "inbound": 0}, "responses": []}
...
def _typewriter_help(textable, value, requiredIndex, counter=1):
	...
	try:
		...
		if counter < len(value):
			app.after(
			 6, lambda: _typewriter_help(textable, value, requiredIndex, counter + 1))
		else:
			try:
				if textable.is_chat_bubble == True:
					is_type_writing_response = False
...
def tryInputDialog(text, title):
	dialog = None

	try:
		dialog = ctk.CTkInputDialog(text=text, title=title)
	except Exception as e:
		print("A known error occured when getting input " + repr(e))
...
class ConfirmDialog(ctk.CTkToplevel):
	...
	def __init__(self, titleText, confirmText, callback):
		...
		try:
			...
			print("A known Error occured setting focus to external window as " +
			      repr(e))
...
class MessagesWindow(ctk.CTkFrame):
	...
	def load_messages(self):
		for messageInfo in messages:
			...
			new_label.setText(
			 messageInfo["content"])  # instantly set the text instead of animating it
...
class MainWindow(ctk.CTkFrame):
	...
	def __init__(self, parent):
		...
		startAdventureButton = ctk.CTkButton(self,
		...
		chatBox = ctk.CTkEntry(self, placeholder_text="Type here...")
		chatBox.bind("<Return>", lambda e: self.sendMessage())

		sendChatIcon = ImageTk.PhotoImage(
		 Image.open("assets/SendChat.png").resize((20, 20),
		                                          Image.Resampling.BILINEAR))
		sendButton = ctk.CTkButton(self,
		                           text="",
		                           image=sendChatIcon,
		                           width=30,
	...
	def sendMessage(self):
		...
		if (is_loading_chat
		    == True) or (is_querying == True) or (is_type_writing_response == True):
			return
...
class NewJourneyWindow(ctk.CTkToplevel):
	...
	def __init__(self):
		...
		label_name = ctk.CTkLabel(
		 self,
		 text="Name your new journey (optional, leave blank for random story):")
		...
		label_context = ctk.CTkLabel(
		 self, text="Additional context (optional, leave blank for random plot):")
	...
	def start_adventure(self):
		...
		newMessages[0][
		 "content"] += " Please begin by explaining and introducing the game to the player and the current setting (with choices)."
...
class App(ctk.CTk):
	...
	def __init__(self):
		...
		sidebarTitleContainer.grid(row=0,
		                           column=0,
		                           padx=10,
		                           pady=(10, 0),
		                           sticky="nsew")
		...
		saveChatIcon = ImageTk.PhotoImage(
		 Image.open("assets/SaveIcon.png").resize((20, 20),
		                                          Image.Resampling.BILINEAR))
		saveChatButton = ctk.CTkButton(self,
		                               image=saveChatIcon,
		                               text="",
		                               width=30,
		                               command=self.makeNewSave)
		saveChatButton.grid(row=0,
		                    column=2,
		                    padx=(0, 10),
		                    pady=(10, 0),
		                    sticky="nsew")
		...
		if not token_exists:
			tokenDialogue = tryInputDialog(
			 text=
			 "No OpenAI Token found. Please enter in a token to continue or else mock data will be used! (ONLY FOR TESTING, DATA IS VERY INACCURATE)"
			)
		...
		if not mock_data:
			ConfirmDialog("API usage warning!",
			              "Mock data is disabled, are you sure you want to proceed?",
			              self.mock_data_result)
	...
	def makeCompletion(self):
		...
		def fetch_openai_response():
			...
			messageInfo = {"role": "assistant", "content": "loading..."}
			...
			if mock_data:
				...
				try:
					...
					ConfirmDialog(
					 "Runtime Error",
					 "There was an exception getting a response from OpenAI: " + repr(e),
					 lambda x: True)
			...
			sessionInfo["tokenUsage"]["inbound"] += response["usage"][
			 "completion_tokens"]
	...
	def saveChat(self, chatName=None):
		...
		chatName = chatName if (not is_empty_string(chatName)) else tryInputDialog(
		 text="Name your chat", title="Save chat").get_input()
		...
		if not is_empty_string(chatName):
			...
			print(
			 f"Added messages to index {chatName}. the messages are {repr(messages)}")
